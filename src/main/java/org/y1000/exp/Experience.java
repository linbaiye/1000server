...
public final class Experience {
    ...
    public Experience(int level, int exp, int gap, int maxExp) {
        this.level = level;
        this.exp = exp;
        this.gap = gap;
        this.maxExp = maxExp;
...
type
   ...
   PTLevelData = ^TLevelData;
    ...
    private static final Experience[] EXPERIENCES = buildArray();

    private static Experience[] buildArray() {
        Experience[] experiences = new Experience[100];
        for (int i = 0; i < 100; i++) {
            int offset = i * 4;
            experiences[i] = new Experience(EXPS[offset],  EXPS[offset + 1], EXPS[offset + 2], EXPS[offset + 3]);
        }
        return experiences;
    }
